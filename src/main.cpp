...
uint256 hashGenesisBlock("0x3d417ab8cc3fd43e5b495678a463d0a1f4b7c86f42e3ef952765fb338ee848f0");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Newyorkcoin: starting difficulty is 1 / 2^12
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 6666); // Newyorkcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 0.6 * 24 * 60 * 60; // Newyorkcoin: 3.5 days
static const int64 nTargetSpacing = 6 * 60; // Newyorkcoin: 2.5 minutes
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Newyorkcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x31ea3a4f0fff4f13ed555bef50316c92f80f9104ffbaef264ec6775091a8be39");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Biden Wins Elections Against Trump";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 66 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("043772ef2b6e973245ff2f591abbe726b02f36e9e71136675a78b359f1200be1c16141b2e2834bc9f5021d67430ec7eedfb89a435f5be1ad35abd641397f816f73") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1622492132;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085568464;

        if (fTestNet)
        {
            block.nTime    = 1622492094;
            block.nNonce   = 385541274;
        }
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x44ab79006b62cfba7a610c04a0cf8694e927bfbf9c646d062e91b9d5a79ef67d"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Newyorkcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("NewyorkcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("NewyorkcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static NewyorkcoinMiner(CWallet *pwallet)
{
    printf("NewyorkcoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running NewyorkcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("NewyorkcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&NewyorkcoinMiner, pwallet));
}
